...
int main(int argc, char** argv)
{
    ...
    LongNumbers MyNum;
    string hexP,hexQ,hexE;
    string ouput;
    BigIntHolder holdP, holdQ, holdE;
    string test_id;
    ...
    if (argc < 3) {
        ...
        //ifstream outputTest("test_0" + to_string(i) + ".out");
        //outputTest >> strD;


        holdP = hexadecimalToDecimal(hexP);
        holdQ = hexadecimalToDecimal(hexQ);
        holdE = hexadecimalToDecimal(hexE);

        LongNumbers::BigNum P, Q, E, One, Phi, D;
        One.Num[0] = 1;
        string tempStr = holdP.toString();
        P = MyNum.StringToArray(tempStr);

        tempStr = holdQ.toString();
        Q = MyNum.StringToArray(tempStr);

        tempStr = holdE.toString();
        E = MyNum.StringToArray(tempStr);


        LongNumbers::BigNum PMinusOne = MyNum.Sub(P, One);
        LongNumbers::BigNum QMinusOne = MyNum.Sub(Q, One);

        Phi = MyNum.Mul(PMinusOne, QMinusOne);
        D = MyNum.Inverse(E, Phi);

        // D = -1
        if (D.negative == true) {
            ouput = MyNum.ToPrint(D);
        }
        else {
            ouput = MyNum.decimalToHexLittleEndian(D);
        }
