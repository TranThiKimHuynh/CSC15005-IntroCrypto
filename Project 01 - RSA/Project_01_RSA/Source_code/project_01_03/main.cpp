...
int main(int argc, char** argv)
{
    ...
    LongNumbers MyNum;
    string hexN, hexK, hexX;
    BigIntHolder holdN, holdK, holdX;
    string output;
    ...
    if (argc < 3) {
        ...
        holdN = hexadecimalToDecimal(hexN);
        holdK = hexadecimalToDecimal(hexK);
        holdX = hexadecimalToDecimal(hexX);

        LongNumbers::BigNum N, K, X, Y;

        string tempStr = holdN.toString();
        N = MyNum.StringToArray(tempStr);
 

        tempStr = holdK.toString();
        K = MyNum.StringToArray(tempStr);
        ...
        Y = MyNum.PwrMod(X, K, N);

        output = MyNum.decimalToHexLittleEndian(Y);
