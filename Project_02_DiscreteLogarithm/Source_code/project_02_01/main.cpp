...
int main(int argc, char** argv)
{
    ...
    LongNumbers MyNum;
    string hexP, hexCountU,hexG;
    vector<string> holdU;
    BigIntHolder holdP, holdCountU, holdTemp,holdG;
    ...
    if (argc < 3) {
        ...
        holdCountU = hexadecimalToDecimal(hexCountU);

        for (int i = 0; i < holdCountU; i++) {
            string temp;
            inputTest >> temp;
            BigIntHolder holdTemp;
            holdTemp = hexadecimalToDecimal(temp);
            holdU.push_back(holdTemp.toString());
        ...
        LongNumbers::BigNum P, G, K, Power, One;
        ...
        for (int i = 0; i < holdU.size(); i++) {
            tempStr = holdU[i];
            K = MyNum.StringToArray(tempStr);

            Power = MyNum.Sub(P, One);
            Power = MyNum.DivLarge(Power, K).Result;
            if (MyNum.EqualZero(MyNum.Sub(MyNum.PwrMod(G, Power, P), One))) {
                res = 0;
                break;
            }
        }
