...
int main(int argc, char** argv)
{
    ...
    LongNumbers MyNum;
    string hexP, hexG, hexA,hexB;
    
    BigIntHolder holdP, holdG, holdA, holdB;

    if (argc < 3) {
        cout << "Not enough Command Line Argument passed!";
    }
    else {
        ifstream testInp(argv[1]);
        ...
        holdA = hexadecimalToDecimal(hexA);
        holdB = hexadecimalToDecimal(hexB);


        LongNumbers::BigNum P, G, a, b, A, B, K, mulAB;
        ...
        tempStr = holdA.toString();
        a = MyNum.StringToArray(tempStr);

        tempStr = holdB.toString();
        b = MyNum.StringToArray(tempStr);

        A = MyNum.PwrMod(G, a, P);
        B = MyNum.PwrMod(G, b, P);

        mulAB = MyNum.Mul(a, b);

        K = MyNum.PwrMod(G, mulAB, P);


        ofstream testOut(argv[2]);
        
        testOut << MyNum.decimalToHexLittleEndian(A)<<endl;
        testOut << MyNum.decimalToHexLittleEndian(B)<<endl;
        testOut << MyNum.decimalToHexLittleEndian(K)<<endl;
        testOut.close();
