...
int main(int argc, char** argv)
{
    ...
    LongNumbers MyNum;
    string hexP, hexG, hexX, hexC1,hexC2;

    BigIntHolder holdP, holdG, holdX, holdC1, holdC2;
    if (argc < 3) {
        ...
        holdX = hexadecimalToDecimal(hexX);
        holdC1 = hexadecimalToDecimal(hexC1);
        holdC2 = hexadecimalToDecimal(hexC2);

        

        LongNumbers::BigNum P, G, X, C1, C2, H, M, S, InverseS, One;
        One.Num[0] = 1;
        ...
        tempStr = holdX.toString();
        X = MyNum.StringToArray(tempStr);

        tempStr = holdC1.toString();
        C1 = MyNum.StringToArray(tempStr);

        tempStr = holdC2.toString();
        C2 = MyNum.StringToArray(tempStr);

        H = MyNum.PwrMod(G, X, P);
        S = MyNum.PwrMod(C1, X, P);
        InverseS = MyNum.Inverse(S, P);
        M = MyNum.Mul(C2, InverseS);
        M = MyNum.PwrMod(M, One, P);


        ofstream outTest(argv[2]);
        string h, m;
        outTest << MyNum.decimalToHexLittleEndian(H)<<endl;
        outTest << MyNum.decimalToHexLittleEndian(M) << endl;
